#!/usr/bin/env python3
"""Крепыш: rembg → квадрат 1024, крупный bust как в Hools."""
...
OUT = ROOT / "public/images/tough-dossier.png"
OUT_LEGACY = ROOT / "public/static/personage/tough.png"
CANVAS = 1024
CONTENT_PAD_RATIO = 0.008
BOTTOM_MARGIN_RATIO = 0.0
TOP_MARGIN_TARGET = 0.012
SCALE_BOOST = 1.11
SHIFT_Y = 24
ALPHA_MIN = 12

SOURCES = [
    ROOT / "assets" / "tough-source.png",
    Path(
        r"C:\Users\vadim\.cursor\projects\c-Anomaly\assets\c__Users_vadim_AppData_Roaming_Cursor_User_workspaceStorage_dc33976521015eca047b7dafcf5cf736_images_photo_2026-06-07_19-22-36-e947c5b7-773e-4255-816c-4d18cfd0a432.png"
    ),
    Path(
        r"C:\Users\vadim\.cursor\projects\c-Anomaly\assets\c__Users_vadim_AppData_Roaming_Cursor_User_workspaceStorage_dc33976521015eca047b7dafcf5cf736_images_F8E7AF90-8ECA-4A6C-8FE3-8AF2B46CCE4D-bd229560-5ad2-43b7-89d1-6c77c6ed1e95.png"
    ),
]
...
def pick_source() -> Path:
    ...
    for p in SOURCES:
        if p.exists():
            return p
    raise FileNotFoundError("No source image for tough dossier")
...
def solidify_fringe(im: Image.Image) -> Image.Image:
    """Убрать ореол фона (чёрный / сепия / жёлтый / коричневый), не трогая персонажа."""
    im = im.convert("RGBA")
    w, h = im.size
    px = im.load()
    for y in range(h):
        for x in range(w):
            ...
            if (
                ...
                px[x, y] = (0, 0, 0, 0)
                continue
            if a < 120 and r >= 150 and g >= 110 and b <= 110 and r >= g >= b:
                px[x, y] = (0, 0, 0, 0)
                continue
            if (
                a < 120
...
def main() -> None:
    src = pick_source()
    if not (ROOT / "assets" / "tough-source.png").exists() and src.exists():
        shutil.copy2(src, ROOT / "assets" / "tough-source.png")
